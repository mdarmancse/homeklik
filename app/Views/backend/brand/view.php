                            ...
                            <div class="page-breadcrumb">
                                <nav aria-label="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><i class="fas fa-tachometer-alt "></i><a href="<?php echo site_url('/') ?>" class="breadcrumb-link"> Dashboard</a></li>
                                        <li class="breadcrumb-item active" aria-current="page"><i class="fa fa-fw fa-star"></i> Brand</li>
                                    </ol>
                                </nav>
                            </div>
                        </div>
                        ...
                        <a type="button" class="btn btn-sm btn-success" href="<?php echo site_url('/brand/add') ?>">Add Brand</a>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="brand_ajax" class="table table-striped table-bordered second" style="width:100%">
                                        <thead>
                                            <tr>
                                                <th>Sl</th>
                                                <th>Name</th>
...
$(function(){
    <?php if(session()->has("brand_success")) { ?>
        Swal.fire({
            icon: 'success',
            title: 'Great!',
            text: '<?= session("brand_success") ?>'
        })
        setTimeout(function() { 
                location.reload();
                }, 1000);   
    <?php } ?>
    <?php if(session()->has("brand_error")) { ?>
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: '<?= session("brand_error") ?>'
        })
        setTimeout(function() { 
                location.reload();
                }, 1000);   
            ...
            callback: function(dsiableConfirm) {
                if (dsiableConfirm) {
                    jQuery.ajax({
                        type: "POST",
                        dataType: "json",
                        url: "<?php echo base_url('brand/status/') ?>"+entity_id + "/" + status,
                        data: {
                            'entity_id': entity_id,
                            'status': status 
                        },
            ...
            callback: function(dsiableConfirm) {
                if (dsiableConfirm) {
                    jQuery.ajax({
                        type: "POST",
                        dataType: "json",
                        url: "<?php echo base_url('brand/delete/') ?>"+entity_id,
                        data: {
                            'entity_id': entity_id, 
                        },
                        success: function(response) {
    ...
    $(document).ready(function() {
        $('#brand_ajax').DataTable({
            ajax: {
                url: "<?php echo base_url('brand/getAjaxData'); ?>",
                type: 'POST',
                dataType: 'json',
                dataSrc: ''
            },
